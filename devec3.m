...
% This code is a version of the implemented algorithm Differential Evolution 
% (Differential Evolution for MATLAB 1996, R. Storn), modified of its 
% original (http://www.icsi.berkeley.edu/~storn/code.html). 
% This version minimize a user-supplied function with respect to x(1:D),
...
for i=1:NP
   pop(i,:) = round(XVmin + rand(1,D).*(XVmax - XVmin));
end
...
%-----Select which vectors are allowed to enter the new population------------
  for i=1:NP
    ...
    tempval = feval(fname,abs(round(ui(i,:))),y);   % check cost of competitor
    nfeval  = nfeval + 1;
...
%        for n=1:D
%          fprintf(1,'best(%d) = %.0f [%.0f , %.0f]\n',n,bestmem(n),auxiliar1(bestmem(n)),auxiliar2(bestmem(n)));
%        end
    end
  end
